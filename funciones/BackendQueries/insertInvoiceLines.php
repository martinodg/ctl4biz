...
//@todo incluir el change languaje
$errorMessage = null;
$conexion = mysqli_connect($Servidor, $Usuario, $Password, $BaseDeDatos) or throw_ajax_system_error("Error: La Funcion insertTempInvoice no puede conectar con la base de datos");

if (isset($_GET['codFacturat'])) {
    $codFacturat = $_GET['codFacturat'];
} else {
    $errorMessage .= "Error. temporary invoice number is mandatory.";
}
...
//Throwing exception on response
if(!empty($errorMessage))
{
    throw_ajax_validation_error($errorMessage);
}
...
if (isset($_GET['docType'])) {
    ...
    if ($doctype == "tempInvoice") {
        ...
        if (isset($_GET['codfamilia'])) {
            $codfamilia = $_GET['codfamilia'];
        } else {
            $errorMessage .= "Error. Item family code is mandatory.";
        }

        if (isset($_GET['codArticulo'])) {
            $codArticulo = $_GET['codArticulo'];
        } else {
            $errorMessage .= "Error. Item  code is mandatory.";
        }

        if (isset($_GET['cantidad'])) {
            $cantidad = $_GET['cantidad'];
        } else {
            $errorMessage .= "Error. Item  quantity is mandatory.";
        }

        if (isset($_GET['precio'])) {
            $precio = $_GET['precio'];
        } else {
            $errorMessage .= "Error. Item  price is mandatory.";
        }

        if (isset($_GET['importe'])) {
            $importe = $_GET['importe'];
        } else {
            $errorMessage .= "Error. total amount is mandatory.";
        }
        ...
        if (isset($_GET['impuesto'])) {
            ...
            $errorMessage .= "Error. Item  TAX is mandatory.";
        }
        //Throwing exception on response
        if(!empty($errorMessage))
        {
        ...
        echo "INSERT INTO $table (codfactura, numlinea, codfamilia, codigo, cantidad, precio, importe, dcto, TAX) VALUE ('$codFacturat','$codLineTmp','$codfamilia','$codArticulo','$cantidad','$precio','$importe','$dscto','$impuesto'";
